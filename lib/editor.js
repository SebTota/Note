...
function deleteCurrentFile() {
    console.log(`Deleting file ${currentFile.filePath}`)
    ...
    fs.unlink(currentFile.filePath, (err) => {
        if (err) console.log(err);
            ...
            $(`div[data-path="${currentFile.relativePath}"]`).remove();
...
function openFile(dirPath, newFile=false, renamedFile=false) {
    ...
    fileNameInput.val(encryption.decryptName(`${currentFile['fileName']}`));
    fileNameInput.off('change').change(function() {
