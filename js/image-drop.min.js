(function() {
    ...
    var ImageDrop = exports.ImageDrop = function() {
        ...
        _createClass(e, [{
            ...
            key: "insert",
            value: function e(t) {
                const fileExt = t.split(',')[0].split('/')[1].split(';')[0];
                const fileName = require('uuid').v4() + "." + fileExt;
                const filePath = userDataPath + '/files/' + fileName;

                console.log(filePath);

                fs.writeFile(filePath, t.split(',')[1], 'base64', function(err) {
                    if (err) {
                        console.log(err);
                    }

                    var a = (this.quill.getSelection() || {}).index || this.quill.getLength();
                    this.quill.insertEmbed(10, 'image', encodeURI("file://" + filePath), Quill.sources.USER)
